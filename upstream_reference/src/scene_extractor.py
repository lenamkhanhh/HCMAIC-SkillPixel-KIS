...
from transnetv2.model import TransNetV2
...
class TransNetV2Torch:
    def __init__(self):
        ...
        weights_path = str(
            Path(__file__).parent / "transnetv2" / "transnetv2-pytorch-weights.pth"
        )
    ...
    def predict_frames(self, frames: np.ndarray):
        ...
        if self.window_buf is None:
            self.window_buf = np.empty((1, 100, *self._input_size), dtype=np.uint8)
...
def process_one_video(video_path: str, model: TransNetV2Torch, output_dir: str):
    that_folder = output_dir + "/" + os.path.basename(video_path)

    os.makedirs(that_folder, exist_ok=True)

    pred_txt = that_folder + "/" + os.path.basename(video_path) + ".predictions.txt"
    scenes_txt = that_folder + "/" + os.path.basename(video_path) + ".scenes.txt"

    if os.path.exists(pred_txt) or os.path.exists(scenes_txt):
        print(f"[SKIP] {video_path} already processed. Skipping.", file=sys.stderr)
        return

    frames, s_pred, a_pred = model.predict_video(video_path)
...
def main():
    ...
    parser.add_argument("--video", help="Path to video directory")
    parser.add_argument("--output", help="Output directory for scene files")
    ...
    for video_path in pbar:
        ...
        process_one_video(video_path, model, output_dir)
