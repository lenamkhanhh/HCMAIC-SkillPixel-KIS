...
# --- Constants ---
EMBEDDING_DIM = 1536  # Fixed dimension for SigLIP-2 Giant
SIGLIP_MODEL_ID = "google/siglip2-giant-opt-patch16-384"  # SigLIP Giant
DATABASE_FILE = "keyframe_embeddings_siglip2.db"
FAISS_INDEX_FILE = "keyframe_faiss_siglip2.index"
...
def process_keyframe_batch(conn, batch_images, batch_keyframe_data, model, processor, device):
    if not batch_images:
        return 0
    try:
        # SigLIP-2 preprocess and features
        ...
        with torch.no_grad():
            try:
                ...
                # Get image embeddings from SigLIP model output
                image_features = outputs.image_embeds
                # Normalize the features for cosine similarity
                image_features = image_features / image_features.norm(dim=-1, keepdim=True)
                embeddings_np = image_features.cpu().detach().numpy()
                
                print(f"✅ SigLIP Giant embedding shape: {embeddings_np.shape}")
                print(f"🎯 Expected dimension: {EMBEDDING_DIM}")
                
                # Verify dimension matches
                if embeddings_np.shape[1] != EMBEDDING_DIM:
                    print(f"⚠️ Warning: Actual dimension {embeddings_np.shape[1]} != Expected {EMBEDDING_DIM}")
                    
            except Exception as e:
                print(f"[Batch] Error extracting SigLIP embeddings: {e}")
                return 0
...
def search_images_by_image_faiss(query_image_path, faiss_index, faiss_to_db_id_map, db_conn, model, processor, device, top_k=5):
    ...
    with torch.no_grad():
        try:
            ...
            # Extract embeddings using SigLIP
            outputs = model(**inputs)
            query_image_embedding = outputs.image_embeds
...
def search_images_by_text_siglip(text_query, faiss_index, faiss_to_db_id_map, db_conn, model, processor, device, top_k=10):
    """Search images using text query with SigLIP-2"""
    if faiss_index is None or faiss_index.ntotal == 0:
        print("FAISS index not available or empty.")
        return []
    ...
    try:
        ...
        distances, faiss_indices = faiss_index.search(text_np, top_k)
...
def main():
    ...
    # --- Load Model ---
    if not _HAS_SIGLIP:
        print("SigLIP not available. pip install -U transformers")
        return
    try:
        print(f"Loading SigLIP Giant: {SIGLIP_MODEL_ID}")
        processor = AutoProcessor.from_pretrained(SIGLIP_MODEL_ID)
        model = SiglipModel.from_pretrained(SIGLIP_MODEL_ID)
        model = model.to(device)
        model.eval()
        print(f"✅ SigLIP Giant loaded. Embedding dim: {EMBEDDING_DIM}")
    except Exception as e:
    ...
    if faiss_index is None or faiss_id_map is None or faiss_index.ntotal == 0:
        ...
        print("    (Powered by SigLIP Giant)")
        print("="*50 + "\n")
        
        while True:
            print("\nOPTIONS:")
            print("1. Search by image (SigLIP)")
            print("1b. Search by text (SigLIP)")
            ...
            if choice == 'q':
                ...
                print("\n--- TEXT SEARCH (SigLIP Giant) ---")
                text_query = input("Enter text query: ")
                if not text_query.strip():
                    print("Empty query")
                    continue
                try:
                    results = search_images_by_text_siglip(text_query, faiss_index, faiss_id_map, db_conn, model, processor, device, top_k=10)
                    if results:
                        print("\n✅ Top results:")
                ...
                try:
                    ...
                    print(f"Model: SigLIP Giant ({EMBEDDING_DIM}D)")
