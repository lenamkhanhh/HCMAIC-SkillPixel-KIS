...
# --- Constants ---
EMBEDDING_DIM = 1024  # Jina v4 single vector dimension
...
def setup_database(db_path=DATABASE_FILE):
    ...
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_video_id ON keyframe_embeddings (video_id)"
    )
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_image_path ON keyframe_embeddings (image_path)"
    )
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_keyframe_n ON keyframe_embeddings (keyframe_n)"
    )
    conn.commit()
...
def insert_keyframe_data_to_db(conn, keyframe_data):
    ...
    try:
        ...
        cursor.execute(
            "SELECT id FROM keyframe_embeddings WHERE image_path = ?",
            (keyframe_data["image_path"],),
        )
        row = cursor.fetchone()
...
def get_all_embeddings_and_db_ids(conn):
    ...
    for row in cursor.fetchall():
        ...
        if embedding_array is not None and embedding_array.shape == (1, EMBEDDING_DIM):
            ...
            data.append(
                {"db_id": db_id, "embedding": embedding_array}
...
def check_if_keyframe_exists_in_db(conn, image_path_to_check):
    cursor = conn.cursor()
    cursor.execute(
        "SELECT COUNT(id) FROM keyframe_embeddings WHERE image_path = ?",
        (image_path_to_check,),
    )
...
def build_and_save_faiss_index(
    db_conn, index_path=FAISS_INDEX_FILE, map_path=FAISS_ID_MAP_FILE
):
    ...
    # Handle regular tensors
        ...
        try:
            ...
            if hasattr(tensor_obj, "cpu"):
                ...
                raise ValueError(
                    f"Cannot convert {type(tensor_obj)} to numpy array: {e}"
                )
...
def process_keyframe_batch_jina(conn, batch_images, batch_keyframe_data, model, device):
    ...
    try:
                ...
                if isinstance(image_embeddings, list):
                    ...
                    if len(image_embeddings) > 0:
                        ...
                        if hasattr(image_embeddings[0], "shape"):
                            print(
                                f"📋 Shape item đầu tiên: {image_embeddings[0].shape}"
                            )
        ...
        print(
            f"✅ THÀNH CÔNG: Đã lưu {count}/{len(batch_keyframe_data)} embeddings trong batch!"
        )
...
def ingest_keyframes_to_db_jina(
    db_conn, model, device, force_reingest_all=False, batch_size=8
):
    ...
    video_dirs = [
        d
        for d in os.listdir(KEYFRAMES_ROOT)
    ...
    for video_id in tqdm(video_dirs, desc="Processing videos"):
        ...
        for keyframe_file in keyframe_files:
            try:
                ...
                if not force_reingest_all and check_if_keyframe_exists_in_db(
                    db_conn, image_path
                ):
                    skipped_count += 1
                    continue
                ...
                timing_info = keyframe_mapping.get(
                    keyframe_n, {"pts_time": 0.0, "fps": 30.0, "frame_idx": 0}
                )
                ...
                if len(batch_images) == batch_size:
                    inserted = process_keyframe_batch_jina(
                        db_conn, batch_images, batch_keyframe_data, model, device
                    )
    ...
    success_rate = (
        (newly_inserted_count / (total_keyframes_processed - skipped_count)) * 100
        if (total_keyframes_processed - skipped_count) > 0
        else 0
    )
    print(f"- Tỷ lệ thành công: {success_rate:.2f}%")


def search_images_by_image_faiss_jina(
    query_image_path, faiss_index, faiss_to_db_id_map, db_conn, model, device, top_k=5
):
...
def search_images_by_text_jina(
    text_query, faiss_index, faiss_to_db_id_map, db_conn, model, device, top_k=10
):
...
def main():
    ...
    if not _HAS_JINA:
        print(
            "Jina Embeddings not available. pip install transformers>=4.52.0 torch>=2.6.0"
        )
        return
    try:
        print(f"Loading Jina Embeddings v4: {JINA_MODEL_ID}")
        model = AutoModel.from_pretrained(
            JINA_MODEL_ID, trust_remote_code=True, torch_dtype=torch.bfloat16
        )
    ...
    if not Path(KEYFRAMES_ROOT).exists():
        ...
        ingest_keyframes_to_db_jina(
            db_conn, model, device, force_reingest_all=FORCE_REPROCESS_ALL_KEYFRAMES
        )
    ...
    if (
        ...
        or not Path(index_file).exists()
        or not Path(map_file).exists()
    ):
        print("Rebuilding FAISS index and map...")
        faiss_index, faiss_id_map = build_and_save_faiss_index(
            db_conn, index_path=index_file, map_path=map_file
        )
    else:
        print("Loading existing FAISS index and map...")
        faiss_index, faiss_id_map = load_faiss_index_and_map(
            index_path=index_file, map_path=map_file
        )

    if faiss_index is None or faiss_id_map is None or faiss_index.ntotal == 0:
        print(
            "Failed to load or build a valid FAISS index. Search will not be available."
        )
    else:
        ...
        while True:
            ...
            if choice == "q":
                ...
                try:
                    ...
                    results = search_images_by_image_faiss_jina(
                        query_img_path,
                        faiss_index,
                        faiss_id_map,
                        db_conn,
                    ...
                    if results:
                        ...
                        for i, res in enumerate(results):
                            ...
                            print(
                                f"  {i+1}. Video: {res['video_id']} | Keyframe: {res['keyframe_n']:03d} | Time: {res['pts_time']:.1f}s"
                            )
                ...
                if not text_query.strip():
                    ...
                    continue
                try:
                    results = search_images_by_text_jina(
                        text_query,
                        faiss_index,
                        faiss_id_map,
                        db_conn,
                    ...
                    if results:
                        ...
                        for rank, res in enumerate(results):
                            similarity_pct = res["similarity"] * 100
                            print(
                                f"  {rank+1}. Video: {res['video_id']} | Keyframe: {res['keyframe_n']:03d} | Time: {res['pts_time']:.1f}s"
                            )
                ...
                if (
                    input(
                        "This will re-process all keyframes. Continue? (y/n): "
                    ).lower()
                    == "y"
                ):
                    print("Re-ingesting all keyframes...")
                    ingest_keyframes_to_db_jina(
                        db_conn, model, device, force_reingest_all=True
                    )
                    print("Rebuilding FAISS index...")
                    faiss_index, faiss_id_map = build_and_save_faiss_index(
                        db_conn, index_file, map_file
                    )
                ...
                try:
                    ...
                    # Count videos
                    cursor.execute(
                        "SELECT COUNT(DISTINCT video_id) FROM keyframe_embeddings"
                    )
                    ...
                    if keyframe_count != faiss_count:
                        ...
                        if (
                            input(
                                "Would you like to rebuild the FAISS index? (y/n): "
                            ).lower()
                            == "y"
                        ):
