...
class ObjectRecord:
    ...
    def __post_init__(self) -> None:
        ...
        if self.bbox is not None and (
            len(self.bbox) != 4
            or any(not math.isfinite(value) or value < 0 for value in self.bbox)
        ):
...
def _merge_manifest_extra(manifest: dict[str, Any], extra: dict[str, Any] | None) -> None:
    ...
    if overlap:
        raise ObjectArtifactError(
            f"manifest_extra cannot override reserved fields: {overlap}"
        )
...
def write_object_artifact(
    ...
    *,
    ...
    keys = [
        (record.frame_uid, normalize_object_label(record.label)) for record in records
    ]
...
def _parse_records(payload: bytes) -> list[ObjectRecord]:
    ...
    for line_number, line in enumerate(lines, start=1):
        try:
            ...
            raise ObjectArtifactError(
                f"invalid object record at line {line_number}"
            ) from exc
    return records
...
class ObjectRetrievalChannel:
    ...
    def from_artifact(
        ...
        return cls(
            load_object_artifact(artifact_dir, dataset_manifest_hash=dataset_manifest_hash)
        )
    ...
    def search(self, text: str, top_k: int = 100) -> list[ChannelHit]:
        ...
        candidates = sorted(
            {
                ...
                for record_idx in self._postings.get(token, [])
            }
        )
        if not candidates:
            return []
        ...
        for record_idx in candidates:
            ...
            matching_labels = [
                token for token in query_tokens if token in frame.tokens
            ]
