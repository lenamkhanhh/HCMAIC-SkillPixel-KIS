...
class ASRRetrievalChannel:
    ...
    def __init__(self, artifact: ASRArtifact) -> None:
        ...
        self._df = Counter(
            token for segment in self._segments for token in set(segment.tokens)
        )
    ...
    def search(self, text: str, top_k: int = 100) -> list[ChannelHit]:
        ...
        candidates = sorted(
            {
                ...
                for segment_idx in self._postings.get(token, [])
            }
        )
        if not candidates:
            return []
        ...
        return [
            ...
            for rank, (segment, score, matching_tokens) in enumerate(
