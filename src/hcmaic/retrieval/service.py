...
class RetrievalService:
    def __init__(
        ...
        self._by_frame_id: dict[str, FrameRecord] = {
            r.frame_id: r for r in artifacts.catalog
        }
        self._row_of: dict[str, int] = {
            fid: i for i, fid in enumerate(artifacts.id_map)
        }
    ...
    def frame_image_path(self, frame_id: str) -> Path:
        ...
        try:
            ...
            raise PermissionError(
                f"Image path for {frame_id} escapes the dataset root"
            ) from exc
        return path
    ...
    def search(self, request: SearchRequest) -> list[SearchResult]:
        ...
        if raw_filter:
            ...
            if not isinstance(raw_filter, list):
                raise ValueError(
                    "filters.video_ids must be a list of video ids or a "
