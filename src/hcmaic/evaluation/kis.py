...
class KISQrelSet:
    ...
    def quality_status(self) -> str:
        return (
            "VALIDATED_ON_HCMAIC"
            if self.source == "hcmaic-official"
            else "UNVALIDATED_ON_HCMAIC"
        )
...
def load_kis_qrels(path: Path, *, source: str = "unknown") -> KISQrelSet:
    ...
    for row_number, row in enumerate(rows, start=1):
        ...
        frame_values = _as_values(
            row.get("relevant_frame_uids", row.get("relevant_frame_ids"))
        )
    ...
    *,
    ...
    for question in questions:
        ...
        invalid = [
            ...
            if result.source_frame_idx < 0 or not result.video_filename
        ]
    ...
    report = {
        ...
        "recall_at": {
            str(cutoff): (hits_at[cutoff] / n_scored if n_scored else None)
            for cutoff in CUTOFFS
        },
...
def run_kis_ablation(
    ...
    *,
    ...
    for mode, enabled in modes.items():
        channels = {
            name: channel if name in enabled else None
            for name, channel in available.items()
        }
