    ...
    with open(path, encoding="utf-8") as f:
        for lineno, line in enumerate(f, start=1):
            ...
            if "query_id" not in data or "text" not in data:
                raise ValueError(
                    f"{path}:{lineno}: query needs 'query_id' and 'text', got {data}"
                )
            queries.append(
                EvalQuery(
...
def evaluate(
    ...
    mode = (
        "deterministic-mock" if provider == "mock" else f"real-{provider}-smoke"
    )
    report: dict[str, Any] = {
        "mode": mode,
        "disclaimer": (
        ...
        "recall_at": {
            str(c): (hits_at[c] / n_scored if n_scored else 0.0) for c in CUTOFFS
        },
