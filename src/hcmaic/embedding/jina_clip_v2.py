...
class RealJinaClipV2EmbeddingProvider(EmbeddingProvider):
    ...
    def __init__(
        ...
        *,
        ...
        try:
            ...
            raise RuntimeError(
                "Jina CLIP v2 requires torch+transformers. Install with: "
                "uv sync --extra clip"
            ) from exc
    ...
    def info(self) -> dict[str, Any]:
        data = super().info()
        preprocessing = (
            "JinaCLIPModel encode_image local-path; "
            "encode_text task=retrieval.query"
        )
        data.update(
            {
        ...
        for start in range(0, len(paths), self._batch):
            ...
            with self._torch.inference_mode():
                features = self._model.encode_image(
                    batch, truncate_dim=self._truncate_dim
                )
