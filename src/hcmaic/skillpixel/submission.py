    ...
    with Path(path).open(encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            if query_id in results:
                raise SubmissionValidationError(
                    [f"results JSONL duplicate query_id {query_id!r}"]
                )
...
def validate_submission_csv(
    ...
    if len(rows) - 1 != len(expected_ids):
        errors.append(
            f"submission query row count {len(rows) - 1} != expected {len(expected_ids)}"
        )
    return SubmissionValidationReport(Path(submission_path), len(rows) - 1, tuple(errors))


def write_results_jsonl(
    results: Mapping[str, Sequence[SkillPixelHit]], output_path: Path
) -> Path:
