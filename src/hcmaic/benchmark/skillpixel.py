...
def benchmark_visual_candidate(
    ...
    *,
    ...
    validation = validate_submission_csv(
        submission_path, config.questions_path, config.corpus_path
    )
    ...
    if not validation.ok:
        raise SkillPixelBenchmarkError(
            f"submission validation failed: {list(validation.errors)}"
        )
        ...
        row.update(
            {
                ...
                "empty_error_queries": report["n_empty_results"]
                + report["n_invalid_results"],
                "metrics": {
    ...
    def load_channels(names: set[str]) -> dict[str, Any]:
        ...
        if "object" in names:
            ...
            channels["object"] = ObjectRetrievalChannel(
                load_object_artifact(
                    config.object_artifact, dataset_manifest_hash=dataset_hash
                )
            )
        return channels
