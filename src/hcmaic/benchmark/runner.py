...
def run_benchmark(config_path: Path, out_dir: Path) -> dict[str, Path]:
    ...
    if not validation.ok:
        raise ValueError(
            f"benchmark dataset has {len(validation.errors)} validation error(s)"
        )
    ...
    summary = {
        **last_report,
        ...
        "repeat_mrr_mean": round(
            statistics.mean(report["mrr"] for report in run_reports), 6
        ),
    ...
    failures = [
        row for row in all_per_query if row.get("first_relevant_rank") is None
    ]
    lines = [
