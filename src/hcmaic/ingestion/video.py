...
def _generate_video_outputs(
    ...
    *,
    ...
    if info.backend == "ffmpeg":
        candidates = _iter_candidates_ffmpeg(
            info, interval_s, max_frames, warnings, out_root
        )
    else:
...
def _append_mapping_rows(out_root: Path, new_rows: list[dict[str, Any]]) -> None:
    existing = [
        r for r in _read_mapping_rows(out_root)
        if r.get("video_id") != new_rows[0]["video_id"]
    ]
...
def collect_video_files(input_path: Path) -> list[Path]:
    ...
    if input_path.is_dir():
        ...
        if not files:
            raise IngestError(
                f"No supported video files ({sorted(SUPPORTED_EXTENSIONS)}) "
